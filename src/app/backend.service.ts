...
export class BackendService {
    ...
    public newTicket(description: string ): Observable<Ticket> {
        ...
        return of(newTicket).pipe(
            delay(randomDelay()),
            tap((ticket: Ticket) => {
                let copyStored = [...this.storedTickets];
                copyStored.push(ticket);
                this.storedTickets = copyStored;
            })
        );
    }
        ...
        if (foundTicket && user) {
            return of(foundTicket).pipe(
                ...
                tap((ticket: Ticket) => {
                    ...
                    ticketCopy.assigneeId = +userId;
                    let copyStored = [...this.storedTickets];
                    copyStored.splice(ticket.id, 1, ticketCopy);
                    this.storedTickets = copyStored;
                })
            );
        }
